...
class Topologies():
    ...
    def generate(self):
        with open('./examples/topologies/template.csc') as file:
            ...
            mote = BeautifulSoup(''.join(self.getMote(i, 'sky2')
                                         for i in range(1, self.nodes+1)), "html.parser")
            soup.find('motetypesky').insert_after(mote)
            #lines = str(soup).split('\n')
            file = open(os.path.join(
                fileDir, f'./../examples/topologies/view-nodes{self.nodes}-Sim{self.index}.csc'), 'w')
            lines = str(soup)
...
def generateGraph(metric, ofs, nodeInterval, folder="sim-1", lifetime=[]):
    ...
    for i in range(len(nodeInterval)):
        ...
        for of in ofs:
            sim = Simulation(f'{path_to}/log-{of}-Sim{i+1}.txt',
                             f'{path_to}/view-nodes{nodeInterval[i]}-Sim{i+1}.csc')
            if(metric != 'Lifetime'):
                print(f'{of}:')
    ...
    print(data)
    print(df)
    filename = f'images/figures/fig-{metric}-nodes.png';
    if(metric == 'PDR'):
        ax = df.plot(kind='bar', rot=0,  ylim=(0, 100),
                ylabel="Packet Delivery Rate (PDR)", xlabel="Number of nodes")
        ax.figure.savefig(filename)
    elif(metric == 'Energy'):
        ax = df.plot(kind='area', stacked=False,
                ylabel="Average energy consumption (j)", xlabel="Number of nodes")
        ax.figure.savefig(filename)
    elif(metric == 'Overhead'):
        ax = df.plot(kind='bar', rot=0,
                ylabel="Control Message Overhead", xlabel="Number of nodes")
        ax.figure.savefig(filename)
    elif(metric == 'Lifetime'):
        ax = df.plot(kind='bar', rot=0, ylabel="Network Lifetime (min)",
                     xlabel="Number of nodes", figsize=(16, 8))
        ax.figure.savefig(filename)
...
def generateGraphTraffic(metric, ofs, ppm, nodes, folder="sim-1", lifetime=[]):
    ...
    for i in range(len(ppm)):
        ...
        for of in ofs:
            sim = Simulation(f'{path_to}/log-{of}-Sim{i+1}.txt',
                             f'examples/{folder}/view-nodes{nodes}-Sim1.csc')
            if(metric != 'Lifetime'):
                print(f'{of}:')
                m = sim.evaluateMetric(metric, limit=lifetime[i])
            else:
    ...
    print(data)
    print(df)
    filename = f'images/figures/fig-{metric}-ppm.png';
    if(metric == 'PDR'):
        ax = df.plot(kind='bar', rot=0,  ylim=(0, 100),
                ylabel="Packet Delivery Rate (PDR)", xlabel="ppm")
        ax.figure.savefig(filename)
    elif(metric == 'Energy'):
        ax = df.plot(kind='area', stacked=False,
                ylabel="Average energy consumption (j)", xlabel="ppm")
        ax.figure.savefig(filename)
    elif(metric == 'Overhead'):
        ax = df.plot(kind='bar', rot=0,
                ylabel="Control Message Overhead", xlabel="ppm")
        ax.figure.savefig(filename)
    elif(metric == 'Lifetime'):
        ax = df.plot(kind='bar', rot=0, ylabel="Network Lifetime (min)",
                     xlabel="ppm", figsize=(16, 8))
        ax.figure.savefig(filename)
