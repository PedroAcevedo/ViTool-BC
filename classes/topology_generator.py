...
def generate(self):
    with open('trafficFiles/topologies/template.csc') as file:
        ...
        file = open(os.path.join(fileDir,f'topologies/view-nodes{self.nodes}-Sim{self.index}.csc'), 'w')
        lines = str(soup)
...
def __init__ (self, index, name, nodes, source, TX, RX, endInterval, startInterval=[0,0], TR=100, IR=120, sink=1):
    self.index = index
    self.name = name
    self.nodes = nodes
    self.source = source
...
def generateGraph(metric, ofs, nodeInterval, folder="sim-6"):
    data = dict()
    for i in range(len(nodeInterval)):
        path_to = f'trafficFiles/sims/{folder}/{nodeInterval[i]}'
        for of in ofs:
            sim = Simulation(f'{path_to}/log-{of}-Sim{i+1}.txt',f'{path_to}/PCAP-{of}-Sim{i+1}.csv',f'view-nodes{nodeInterval[i]}-Sim{i+1}.csc');
...
# generateGraph('PDR', ['mrhof', 'of0'],[20])
